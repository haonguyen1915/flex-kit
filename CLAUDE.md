...
| `config.py` | resolve config: global `~/.flex-kit/config.json` overlaid by project `.flexkit/flexkit.config.json` (hosts, skillsDir, agentsDir, commandsDir, docsDir, notify, codexModel, codexEffort). `load_config` requires a project; `resolve_config` tolerates its absence (global+defaults) |
