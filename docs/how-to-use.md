...
```bash
cd my-project
flex-kit init      # scaffolds SOURCE only; run `gen` (or `init --gen`) to build hosts
flex-kit gen       # produce the host surfaces (.claude/, .agents/, .codex/)
```

`init` does one job - set up `.flexkit/` (your source). It generates nothing by
default; `flex-kit gen` (or `flex-kit init --gen` in one step) builds the host
surfaces from it:

```
.flexkit/                      # SOURCE - created by init, you edit this
  flexkit.config.json
  skills/   process-verify-fix-loop/  process-decision-interview/  process-navigator/  process-planning-methodology/
  agents/   planner.md  reviewer.md  implementer.md  tester.md
  commands/ flex-plan.md  flex-implement.md  flex-fix.md  ...
.claude/                       # GENERATED by gen - never edit
...
After `gen`, **open Claude Code in this directory**. The hooks are wired.
...
**Upgraded flex-kit and want the newer prompts?** `flex-kit init --update` refreshes
flex-kit's own content - the base agents/skills/commands **and every installed pack** - to
the installed version, overwriting it. Only your truly custom items (skills/agents flex-kit
doesn't ship) are left untouched. Then `flex-kit gen` (or `--update --gen`) to rebuild the
hosts.
