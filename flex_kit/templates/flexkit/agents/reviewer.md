...
Write two files - `handoffs/` at the **repo root**, `reports/` under the **active plan dir**
(`plans/active/<id>/` from `flex-kit status`, or the repo root if there is no active plan):
